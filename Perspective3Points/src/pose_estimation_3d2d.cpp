...
int main(int argc, char **argv) {
  ...
  Mat r, t, inliers;
  solvePnPRansac(pts_3d, pts_2d, K, Mat(), r, t, false, 100, 8.0, 0.99, inliers,
                 SOLVEPNP_P3P);
  Mat R;
  cv::Rodrigues(r, R);
  ...
  cout << "solve p3p in opencv cost time: " << time_used.count() << " seconds."
       << endl;
  ...
  cout << "solve p3p by gauss newton cost time: " << time_used.count()
       << " seconds." << endl;
  ...
  t2 = chrono::steady_clock::now();
  time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
  cout << "solve p3p by g2o cost time: " << time_used.count() << " seconds."
       << endl;
  return 0;
}
